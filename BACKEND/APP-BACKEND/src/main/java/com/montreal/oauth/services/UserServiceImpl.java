...
//            user.setCreatedBy(currentUser);
    ...
    public Message passwordRecovery(String email) {
        ...
        String linkParse = link
                .replace("=", "-X-")
                .replace("+", "-K-")
                .replace("/", "-W-");
        String linkEmail = "http://localhost:4202/reset-password?link="+linkParse;
        ...
        return  messageService.createMessageBuilder( MessageType.MSG_OK, "Link de recuperação gerado com sucesso!", this.addSingleObj("link", "")).build();
    }
