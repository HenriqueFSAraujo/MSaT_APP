...
interface AssociarLocalizadorProps {
...
const AssociarLocalizador: React.FC<AssociarLocalizadorProps> = ({
  selectedVehicle,
  onClose,
  recharge,
  setRecharge,
      ...
      mandatoryAssociationService
        ...
        .then((data) => {
          const collectionCompany = data?.find(
            (association) => association.company.company_type === 'DADOS_LOCALIZADOR'
          );
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (isEditing && existingAssociation) {
        // Deleta a associação anterior do tipo DADOS_LOCALIZADOR
      ...
      // Filtra para encontrar a nova associação do tipo DADOS_LOCALIZADOR
  ...
  return (
    <StyledForm onSubmit={handleSubmit(onSubmit)}>
      ...
      {existingAssociation && !isEditing ? (
        <>
          <CardComponet title="Localizador Associado">
            <CardContainerRow>
              <ContainerField>
          ...
          <CardContainerRow>
              ...
              onClick={() => {
                if (selectedVehicle?.id) {
                  vehicleStatusService
                    .updateVehicleStatus({
                      vehicleId: selectedVehicle.id,
                      status: 'LOCALIZADOR_ACIONADO',
                      stage: 'BUSCA_PELO_VEICULO',
                    })
                    .then(() => {
                      alert('Localizador acionado com sucesso!');
                    })
                    .catch((error) => {
                      console.error('Erro ao acionar localizador:', error);
                      alert('Erro ao acionar localizador. Tente novamente.');
                    })
                    .finally(() => {
            ...
            >
              Acionar o Localizador
            </AcionarCompanyButton>
          </CardContainerRow>
        </>
      ) : (
        <CardComponet title={isEditing ? 'Alterar Localizador' : 'Associar ao Localizador'}>
          <InputGroupContainer>
            <ContainerInput>
              <Label>Selecione o Localizador</Label>
              <Controller
                name="company"
                control={control}
                render={({ field, fieldState: { error } }) => (
                  <InputComponet {...field} select fullWidth variant="outlined" error={!!error}>
                    {companies
                      .filter((item) => item.company_type === 'DADOS_LOCALIZADOR')
                      .map((item) => (
      ...
      {watchCompany && (isEditing || !existingAssociation) && (
        <CardComponet title="Detalhes do Localizador">
          <CardContainerRow>
            <ContainerField>
              <TitleField>Nome</TitleField>
              <ContainerValueField>
...
export default memo(AssociarLocalizador);
