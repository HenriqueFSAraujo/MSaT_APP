...
const AssociarEsCob: React.FC<AssociarEsCobProps> = ({
  selectedVehicle,
  onClose,
  setRecharge,
  recharge,
      ...
      mandatoryAssociationService
        ...
        .then((data) => {
          const collectionCompany = data?.find(
            (association) => association.company.company_type === 'DADOS_ESCRITORIO_COBRANCA'
          );
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (isEditing && existingAssociation) {
        // Deleta a associação anterior do tipo DADOS_ESCRITORIO_COBRANCA
      ...
      await mandatoryAssociationService.createMandatoryAssociation({
        ...
        companyType: 'DADOS_ESCRITORIO_COBRANCA',
      });
      ...
      // Filtra para encontrar a nova associação do tipo DADOS_ESCRITORIO_COBRANCA
  ...
  return (
    <StyledForm onSubmit={handleSubmit(onSubmit)}>
      ...
      {existingAssociation && !isEditing ? (
        <>
          <CardComponet title="EsCob Associado">
            <CardContainerRow>
              <ContainerField>
          ...
          <CardContainerRow>
            <AcionarCompanyButton
              variant="contained"
              onClick={() => {
                alert('EsCob acionado com sucesso!');
        ...
        <CardComponet title={isEditing ? 'Alterar EsCob' : 'Associar ao EsCob'}>
          <InputGroupContainer>
            <ContainerInput>
              <Label>Selecione o EsCob</Label>
              <Controller
                name="company"
                control={control}
                render={({ field, fieldState: { error } }) => (
                  <InputComponet {...field} select fullWidth variant="outlined" error={!!error}>
                    {companies
                      .filter((item) => item.company_type === 'DADOS_ESCRITORIO_COBRANCA')
                      .map((item) => (
      ...
      {watchCompany && (isEditing || !existingAssociation) && (
        <CardComponet title="Detalhes do EsCob">
          <CardContainerRow>
            <ContainerField>
              <TitleField>Nome</TitleField>
              <ContainerValueField>
...
export default memo(AssociarEsCob);
