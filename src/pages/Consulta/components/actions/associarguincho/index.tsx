...
interface AssociarGuinchoProps {
...
const AssociarGuincho: React.FC<AssociarGuinchoProps> = ({
  selectedVehicle,
  onClose,
  recharge,
  setRecharge,
      ...
      mandatoryAssociationService
        ...
        .then((data) => {
          const collectionCompany = data?.find(
            (association) => association.company.company_type === 'DADOS_GUINCHO'
          );
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (isEditing && existingAssociation) {
        // Deleta a associação anterior do tipo DADOS_GUINCHO
      ...
      // Filtra para encontrar a nova associação do tipo DADOS_GUINCHO
  ...
  return (
    <StyledForm onSubmit={handleSubmit(onSubmit)}>
      ...
      {existingAssociation && !isEditing ? (
        <>
          <CardComponet title="Guincho Associado">
            <CardContainerRow>
              <ContainerField>
                ...
                if (selectedVehicle?.id) {
                  vehicleStatusService
                    .updateVehicleStatus({
                      ...
                      status: 'GUINCHO_ACIONADO',
                    })
                    .then(() => {
                      alert('Guincho acionado com sucesso!');
                      setRecharge(!recharge);
                    })
                    .catch((error) => {
                      console.error('Erro ao acionar localizador:', error);
                      alert('Erro ao acionar Guincho. Tente novamente.');
                    });
                }
              }}
            >
        ...
        <CardComponet title={isEditing ? 'Alterar Guincho' : 'Associar ao Guincho'}>
          <InputGroupContainer>
            <ContainerInput>
              <Label>Selecione o Guincho</Label>
              <Controller
                name="company"
                control={control}
                render={({ field, fieldState: { error } }) => (
                  <InputComponet {...field} select fullWidth variant="outlined" error={!!error}>
                    {companies
                      .filter((item) => item.company_type === 'DADOS_GUINCHO')
                      .map((item) => (
      ...
      {watchCompany && (isEditing || !existingAssociation) && (
        <CardComponet title="Detalhes do Guincho">
          <CardContainerRow>
            <ContainerField>
              <TitleField>Nome</TitleField>
              <ContainerValueField>
...
export default memo(AssociarGuincho);
